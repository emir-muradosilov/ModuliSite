...
from core.models import SiteSettings, Product, Category
...
def product_detail(request, category_slug, product_slug):
    ...
    # Здесь вы можете передать любые дополнительные данные
    # Например, похожие товары, хлебные крошки и т.д.
...
def service_page(
    ...
    products = Product.objects.filter(is_active=True, service_template=template)[:8]
    if not products:
         site_settings = SiteSettings.load()
         products = Product.objects.filter(is_active=True, site_settings=site_settings)[:8]
